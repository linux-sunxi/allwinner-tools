...
int setup_mixer(int mixer_num)
{
   ...
   if (script_fetch("mic", "volume", &volume, 1) || 
    ...
    tinymix_set_value(mixer, 2, 0);//mixpas off               
	...
	tinymix_set_value(mixer, 25, mic2_gain);//mic2 amp 41DB
	tinymix_set_value(mixer, 26, mic1_gain);//mic1 amp 35DB 0:0db,1:35db,2:38db,3:41db
	if(mic1_used&&mic2_used) input_source= 4;//????mic1,?ұ?mic2
    if(mic1_used&&!mic2_used) input_source=2;// ???Ҷ?Ϊmic1
    if(!mic1_used&&mic2_used) input_source=3;// ???Ҷ?Ϊmic2
    if(!mic1_used&&!mic2_used) input_source=4;//????mic1,?ұ?mic2
	tinymix_set_value(mixer, 23, input_source);//????ADC????Դ  2Ϊmic1??3Ϊmic2,4Ϊ????mic1,?ұ?mic2
	tinymix_set_value(mixer, 27, 1);//VMic enable 
	tinymix_set_value(mixer, 30, 1);//ADC Left Channel enable
	tinymix_set_value(mixer, 31, 1);//ADC Right Channel enable
	mixer_close(mixer);
